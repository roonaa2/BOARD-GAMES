...
// Helper method to check cell content
bool NumericalBoard::is_cell_empty(int x, int y) const {
    if (x < 0 || x >= rows || y < 0 || y >= columns) {
        return false;
    }
...
Move<char>* NumericalUI::get_move(Player<char>* player) {
    ...
    if (player->get_type() == PlayerType::HUMAN) {
        ...
        while (!valid_input) {
            ...
            // Check if cell is occupied - using the helper method
            if (!num_board->is_cell_empty(x, y)) {
        ...
        while (!found) {
            ...
            // Check if cell is empty using the helper method
            if (num_board->is_cell_empty(x, y)) {
